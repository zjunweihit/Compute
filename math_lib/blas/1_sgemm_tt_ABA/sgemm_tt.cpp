...
int main()
{
    ...
    CHECK_HIP_ERROR(hipMalloc(&dd, nd * sizeof(float)));

    // C(m x n) * AT(k x m) = D(m x m) in GPU
    // n == k
    CHECK_HIPBLAS_ERROR(hipblasSgemm(handle,
    ...
    CHECK_HIP_ERROR(hipMemcpy(hd.data(), dd, sizeof(float) * nd, hipMemcpyDeviceToHost));
