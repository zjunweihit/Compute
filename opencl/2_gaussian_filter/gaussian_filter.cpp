                       ...
                       int width, int height)
{
    ...
    errRet |= clSetKernelArg(kernel, 2, sizeof(cl_sampler), &sampler);
    errRet |= clSetKernelArg(kernel, 3, sizeof(cl_int), &width);
    errRet |= clSetKernelArg(kernel, 4, sizeof(cl_int), &height);
    if (errRet != CL_SUCCESS) {
...
int main(int argc, char **argv)
{
    ...
    sampler = clCreateSampler(ctx,
                              CL_FALSE, // Non-normalized
    ...
    // [4] create a program
    program = CreateProgram(ctx, dev, "gaussian_filter.cl");
    if (program == NULL) {
    ...
    kernel = CreateKernel(program, memObjects, sampler, width, height);
    if (kernel == NULL) {
