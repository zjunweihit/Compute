		...
		case CL_DEVICE_SINGLE_FP_CONFIG:
			{
				...
				appendBitfield<long unsigned int>(
					*(reinterpret_cast<long unsigned int*>(info)),
					CL_FP_DENORM,
					"CL_FP_DENORM",
					fpType);
		...
		case CL_DEVICE_GLOBAL_MEM_CACHE_TYPE:
			{
				...
				appendBitfield<unsigned int>(
					*(reinterpret_cast<unsigned int*>(info)),
					CL_NONE,
					"CL_NONE",
					memType);
				appendBitfield<unsigned int>(
...
void displayInfo(void)
{
	...
	for (unsigned int i = 0; i < numPlatforms; i++) {
		...
		{
			InfoDevice<long unsigned int>::display(
				devices[j],
				CL_DEVICE_TYPE,
				"CL_DEVICE_TYPE");

			InfoDevice<unsigned int>::display(
				devices[j],
				CL_DEVICE_VENDOR_ID,
				"CL_DEVICE_VENDOR_ID");
