...
int main(int argc, char **argv)
{
	...
	int *ptr = NULL;
	status = hsa_memory_allocate(region, SMALL_SIZE, (void **)&ptr);
	check(Allocate small system memory, status);
	memset(ptr, 0xA, SMALL_SIZE);
	DPT("memory value is 0x%X\n", ptr[16]);
	status = hsa_memory_free(ptr);
	check(Free small system memory, status);

	status = hsa_memory_allocate(region, SMALL_SIZE, (void **)&ptr);
	check(Allocate big system memory, status);
	memset(ptr, 0xB, SMALL_SIZE);
	DPT("memory value is 0x%X\n", ptr[16]);
	status = hsa_memory_free(ptr);
	check(Free big system memory, status);
	...
	status = hsa_memory_allocate(region, SMALL_SIZE, (void **)&ptr);
	check(Allocate small local memory, status);
	memset(ptr, 0xC, SMALL_SIZE);
	DPT("memory value is 0x%X\n", ptr[16]);
	status = hsa_memory_free(ptr);
	check(Free small local memory, status);

	status = hsa_memory_allocate(region, LARGE_SIZE, (void **)&ptr);
	check(Allocate big local memory, status);
	memset(ptr, 0xD, SMALL_SIZE);
	DPT("memory value is 0x%X\n", ptr[16]);
	status = hsa_memory_free(ptr);
	check(Free big local memory, status);
