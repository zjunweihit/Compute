...
static hsa_status_t test_h2h(hsa_agent_t gpu_agent, hsa_agent_t cpu_agent, size_t size)
{
	hsa_status_t status;
	hsa_region_t region;
	char *src = NULL;
	...
	memset(src, 0xA, size);
	DPT("memory src is 0x%X\n", src[16]);

	status = hsa_memory_allocate(region, size, (void **)&dst);
	check(Allocate system memory dst, status);
	memset(dst, 0xB, size);
	DPT("memory dst is 0x%X\n", dst[16]);

	memcpy(dst, src, 10);
	DPT("memory dst [8] 0x%X [16] 0x%X\n", dst[8], dst[16]);

	hsa_signal_t complete_signal;
	status = hsa_signal_create(1, 0, NULL, &complete_signal);
	check(create complete signal, status);
	status = hsa_amd_memory_async_copy(dst, cpu_agent, src, cpu_agent,
					20, 0, NULL, complete_signal);
	...
	check(hsa memory async copy, status);
	DPT("memory dst [16] 0x%X  [32] 0x%X\n", dst[16], dst[32]);
...
static hsa_status_t test_h2d(hsa_agent_t gpu_agent, hsa_agent_t cpu_agent, size_t size)
{
	hsa_status_t status;
	...
	status = hsa_signal_create(1, 0, NULL, &complete_signal);
	check(create complete signal, status);
	status = hsa_amd_memory_async_copy(dst, gpu_agent, src, cpu_agent,
...
static hsa_status_t test_d2h(hsa_agent_t gpu_agent, hsa_agent_t cpu_agent, size_t size)
{
	hsa_status_t status;
	hsa_region_t sys_region, loc_region;
	char *src = NULL;
	...
	status = hsa_signal_create(1, 0, NULL, &complete_signal);
	check(create complete signal, status);
	status = hsa_amd_memory_async_copy(dst, cpu_agent, src, gpu_agent,
					   16, 0, NULL, complete_signal);
	while (hsa_signal_wait_scacquire(complete_signal, HSA_SIGNAL_CONDITION_LT, 1,
				UINT64_MAX, HSA_WAIT_STATE_ACTIVE));
	hsa_signal_destroy(complete_signal);
...
static hsa_status_t test_d2d(hsa_agent_t gpu_agent, hsa_agent_t cpu_agent, size_t size)
{
	hsa_status_t status;
	hsa_region_t region;
	char *src = NULL;
	...
	status = hsa_signal_create(1, 0, NULL, &complete_signal);
	check(create complete signal, status);
	status = hsa_amd_memory_async_copy(dst, gpu_agent, src, gpu_agent,
			16, 0, NULL, complete_signal);
	while (hsa_signal_wait_scacquire(complete_signal, HSA_SIGNAL_CONDITION_LT, 1,
				UINT64_MAX, HSA_WAIT_STATE_ACTIVE));
	hsa_signal_destroy(complete_signal);
...
int main(int argc, char **argv)
{
	...
	status = hsa_iterate_agents(get_cpu_agent_cb, &cpu_agent);
	if (status == HSA_STATUS_INFO_BREAK) status = HSA_STATUS_SUCCESS;
	check(Find cpu agent, status);
	...
	status = test_h2h(gpu_agent, cpu_agent, SMALL_SIZE);
	check(=== test h2h ===, status);

	status = test_h2d(gpu_agent, cpu_agent, LARGE_SIZE);
	check(=== test h2d ===, status);

	status = test_d2h(gpu_agent, cpu_agent, LARGE_SIZE);
	check(=== test d2h ===, status);

	status = test_d2d(gpu_agent, cpu_agent, LARGE_SIZE);
