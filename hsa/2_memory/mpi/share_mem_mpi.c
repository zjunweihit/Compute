...
void export_memory(void)
{
	...
	printf("[export] pid: %d, send data: 0x%X\n",
			getpid(), handle.handle[0]);
...
void import_memory(void)
{
	...
	printf("[import] pid: %d, receive data: 0x%X\n",getpid(), handle.handle[0]);
...
int main(int argc, char **argv)
{
	...
	if (world_size != 2) {
		fprintf(stderr, "World size should be 2\n");
